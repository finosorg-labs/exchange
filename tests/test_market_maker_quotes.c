...
TEST(test_market_maker_quotes_batch_1000) {
    ...
    ASSERT_NOT_NULL(mid_prices);
    ASSERT_NOT_NULL(inventories);
    ASSERT_NOT_NULL(volatilities);
    ASSERT_NOT_NULL(arrival_rates);
    ASSERT_NOT_NULL(bid_prices);
    ASSERT_NOT_NULL(ask_prices);
...
TEST(test_reservation_price_invalid_args) {
    ...
    fc_status_t status = fc_market_maker_reservation_price(
        NULL, mid_prices, inventories, volatilities, 0.1, 1.0, 1
    );
    ASSERT_EQ(status, FC_ERR_INVALID_ARG);
