...
TEST(test_vwap_dev_basic) {
    ...
    double vwap[] = {
        98.0, 102.0,  /* Symbol 0: windows 0, 1 */
        100.0, 103.0, /* Symbol 1: windows 0, 1 */
        100.0, 98.0   /* Symbol 2: windows 0, 1 */
    };
    double sigma[] = {
        2.0, 1.0,     /* Symbol 0 */
        5.0, 2.0,     /* Symbol 1 */
...
TEST(test_vwap_update_basic) {
    ...
    fc_status_t status = fc_ex_sig_vwap_update(
        pv_acc, v_acc, pv_comp, v_comp, vwap_out, price, volume, n
    );
...
TEST(test_vwap_update_multiple_trades) {
    ...
    fc_status_t status = fc_ex_sig_vwap_update(
        pv_acc, v_acc, pv_comp, v_comp, vwap_out, price1, volume1, n
    );
    FC_TEST_ASSERT_EQ(status, FC_OK);
    FC_TEST_ASSERT_DOUBLE_EQ(vwap_out[0], 100.0, 1e-10);
    ...
    status = fc_ex_sig_vwap_update(
        pv_acc, v_acc, pv_comp, v_comp, vwap_out, price2, volume2, n
    );
    FC_TEST_ASSERT_EQ(status, FC_OK);
    ...
    status = fc_ex_sig_vwap_update(
        pv_acc, v_acc, pv_comp, v_comp, vwap_out, price3, volume3, n
    );
    FC_TEST_ASSERT_EQ(status, FC_OK);
...
TEST(test_vwap_dev_large_batch) {
    ...
    FC_TEST_ASSERT(price != NULL && vwap != NULL && sigma != NULL && z_out != NULL);
