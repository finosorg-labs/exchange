...
TEST(test_feature_basic_single_symbol) {
    ...
    fc_status_t status =
        fc_ex_sig_feature_extract(features, bid_p, bid_q, ask_p, ask_q, 1, n_levels);
...
TEST(test_feature_multi_symbol) {
    ...
    fc_status_t status =
        fc_ex_sig_feature_extract(features, bid_p, bid_q, ask_p, ask_q, n_symbols, n_levels);
...
TEST(test_feature_null_pointers) {
    ...
    FC_TEST_ASSERT_EQ(
        fc_ex_sig_feature_extract(NULL, bid_p, bid_q, ask_p, ask_q, 1, 5), FC_ERR_INVALID_ARG
    );
    FC_TEST_ASSERT_EQ(
        fc_ex_sig_feature_extract(features, NULL, bid_q, ask_p, ask_q, 1, 5), FC_ERR_INVALID_ARG
    );
    FC_TEST_ASSERT_EQ(
        fc_ex_sig_feature_extract(features, bid_p, NULL, ask_p, ask_q, 1, 5), FC_ERR_INVALID_ARG
    );
    FC_TEST_ASSERT_EQ(
        fc_ex_sig_feature_extract(features, bid_p, bid_q, NULL, ask_q, 1, 5), FC_ERR_INVALID_ARG
    );
    FC_TEST_ASSERT_EQ(
        fc_ex_sig_feature_extract(features, bid_p, bid_q, ask_p, NULL, 1, 5), FC_ERR_INVALID_ARG
    );
}
...
TEST(test_feature_invalid_size) {
    ...
    FC_TEST_ASSERT_EQ(
        fc_ex_sig_feature_extract(features, bid_p, bid_q, ask_p, ask_q, 0, 5), FC_ERR_INVALID_ARG
    );
    FC_TEST_ASSERT_EQ(
        fc_ex_sig_feature_extract(features, bid_p, bid_q, ask_p, ask_q, 1, 0), FC_ERR_INVALID_ARG
    );
    FC_TEST_ASSERT_EQ(
        fc_ex_sig_feature_extract(features, bid_p, bid_q, ask_p, ask_q, 1, -1), FC_ERR_INVALID_ARG
    );
    FC_TEST_ASSERT_EQ(
        fc_ex_sig_feature_extract(
            features, bid_p, bid_q, ask_p, ask_q, 1, FC_EX_FEATURE_MAX_LEVELS + 1
        ),
        FC_ERR_INVALID_ARG
    );
}
...
TEST(test_feature_price_gaps) {
    ...
    /* Level 1: features[23] = bid_gap = 100.0 - 99.8 = 0.2, features[24] = ask_gap = 100.7 - 100.5
     * = 0.2 */
    FC_TEST_ASSERT_DOUBLE_EQ(features[23], 0.2, 1e-10);
    FC_TEST_ASSERT_DOUBLE_EQ(features[24], 0.2, 1e-10);

    /* Level 2: features[31] = bid_gap = 99.8 - 99.5 = 0.3, features[32] = ask_gap = 101.0 - 100.7 =
     * 0.3 */
...
TEST(test_feature_core_only) {
    ...
    fc_status_t status =
        fc_ex_sig_feature_extract_core(features, bid_p, bid_q, ask_p, ask_q, n_symbols, n_levels);
...
TEST(test_feature_batch_sizes) {
    ...
    for (size_t s = 0; s < n_sizes; s++) {
        ...
        fc_status_t status =
            fc_ex_sig_feature_extract(features, bid_p, bid_q, ask_p, ask_q, n, n_levels);
...
TEST(test_feature_min_max_levels) {
    ...
    /* Test maximum levels (n_levels = FC_EX_FEATURE_MAX_LEVELS) */
    {
        ...
        fc_status_t status = fc_ex_sig_feature_extract(
            features, bid_p, bid_q, ask_p, ask_q, 1, FC_EX_FEATURE_MAX_LEVELS
        );
        FC_TEST_ASSERT_EQ(status, FC_OK);
