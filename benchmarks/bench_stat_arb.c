...
static void bench_coint_beta_func(void* user_data) {
    ...
    fc_ex_strat_coint_beta(data->beta_out, data->log_pa, data->log_pb, data->n_pairs, data->window);
}
...
static void bench_coint_spread_z_func(void* user_data) {
    ...
    fc_ex_strat_coint_spread_z(
        data->spread_out, data->z_out, data->pa, data->pb, data->beta, data->states, data->n_pairs
    );
}
...
static void bench_zscore_state_operations(void) {
    ...
    printf(
        "StatArb/ZScoreInit                               %6d          %8.2f ns/op\n", 1, init_ns
    );
    ...
    printf(
        "StatArb/ZScoreReset                               %6d          %8.2f ns/op\n",
        1000,
        reset_ns
    );
    ...
    printf(
        "StatArb/ZScoreFree                               %6d          %8.2f ns/op\n", 1, free_ns
    );
}
...
static void bench_incremental_vs_batch(void) {
    ...
    printf(
        "  Incremental (%zu updates): %.2f μs/op (%.2f ns per pair per update)\n",
        n_updates,
        incremental_us,
        (double) incremental_ns / n_updates / n_pairs
    );
    ...
    printf(
        "  Batch recalculation: %.2f μs (%.2f MB/s, %.2f ns per element)\n",
        batch_us,
        throughput_mb_s,
        (double) batch_ns / (n_pairs * window_size)
    );
